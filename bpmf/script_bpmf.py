...
R_test = ranking_matrix(N, M, datapath + "random_num.csv")
print('End')
...
for D in D_list:
    ...
    U_in = np.zeros((D,
                     N))  # how should we get the starting values of them??  MAYBE THIS MAY GIVE NOT GOOD VALUES OF COVARIANCE MATRIX
    V_in = np.zeros((D, M))
    ...
    row = pd.DataFrame.from_items(
        [('D', D), ('train_err', train_err), ('test_err', test_err), ('train_step', train_epochs)])
